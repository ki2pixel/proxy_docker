...
- **Tierhive bloque le port 1337 sortant** : les proxys statiques Proxiware écoutent tous sur 1337 (port fixe), donc une VM Tierhive ne peut jamais les joindre directement — timeout TCP tous ports vers les IPs Proxiware, ICMP OK, reste d'Internet OK. Symptôme identique sur plusieurs IPs sources/localisations → ce n'est PAS un problème d'IP source, de swap ou de config. **Solution** : relais SOCKS5 sur la VM Azure (`scripts/proxiware_relay.py`, services `proxiware-relay-{1..4}`, ports 10801–10804 en TCP+UDP avec SOCKS5 UDP ASSOCIATE pour QUIC, `LimitNOFILE=65535`, `TasksMax=4096`, keepalive agressif à 90s et timeout select 300s anti-leak de sockets) ; les gateways Tierhive pointent vers `68.210.184.174:1080{n}` via `.env2` (USER/PASS vides). Voir README « Relais SOCKS5 Azure → Proxiware ».
- **Repocket (état zombie)** : après une déconnexion ou micro-coupure de socket, le client Node.js de Repocket ne crashe pas mais reste bloqué indéfiniment en échec silencieux (`Failed to create connection: undefined` / `Peer not found (406)`). Docker ne le relance jamais automatiquement par défaut. **Solution** : le watchdog `scripts/repocket_watchdog.sh` (timer systemd `repocket-watchdog.timer` toutes les 5 min sur Azure et Tierhive) détecte cet état et applique un `docker restart` ciblé.
- **Pawns.app (crash loop sur IP non-résidentielle)** : Pawns.app rejette formellement les plages de proxys ISP commerciaux Proxiware (`non_residential_ip` / `ssh: tcpip-forward request denied by peer`) et termine avec exit 1, provoquant une boucle perpétuelle de crash/restart toutes les minutes qui sature la mémoire et le CPU des VMs à 1 Go. **Solution** : retirer `pawns` de `COMPOSE_PROFILES` (`wipter,honeygain,packetstream,repocket...`) lorsqu'on utilise ces proxys.
...
- **Honeygain** : après un redémarrage, `Device with this name is already active` temporaire — auto-résorbable en quelques minutes. L'erreur `Network Unusable` survient lors d'une détection proxy ou d'un refroidissement suite à un fort trafic cumulé / multi-devices sur le même ASN.
- **Validation IP plateformes** : Pawns/Honeygain peuvent rejeter une IP temporairement (`tcpip-forward denied` / `Network Unusable`) — délai plateforme ou rejet de politique réseau.
