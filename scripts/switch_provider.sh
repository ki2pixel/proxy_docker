...
if [ -z "$TARGET" ]; then
  ...
  echo "  1) TraffMonetizer (traffmonetizer)"
  ...
  case "$CHOICE" in
    0) TARGET="none" ;;
    1) TARGET="traffmonetizer" ;;
    2) TARGET="honeygain" ;;
    3) TARGET="packetstream" ;;
    4) TARGET="pawns" ;;
    5) TARGET="repocket" ;;
    6) TARGET="all" ;;
    *) TARGET="traffmonetizer" ;;
  esac
fi
...
case "$TARGET" in
  ...
  traffmonetizer|tm|traffic) TARGET="traffmonetizer" ;;
  ...
  *)
    ...
    echo "    Options valides : none | traffmonetizer | honeygain | packetstream | pawns | repocket | all"
    exit 1
    ;;
esac
...
if [ "$TARGET" != "all" ] && [ "$TARGET" != "none" ]; then
  for G in $GATEWAYS; do
    for TYPE in traffmonetizer honeygain packetstream pawns repocket; do
