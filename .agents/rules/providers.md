...
description: Comportement des providers de monétisation — EarnFM, Honeygain, PacketStream, Pawns.app, Repocket — et leurs pièges
---
...
| **EarnFM** | `earnfm/earnfm-client:latest` | `GW{n}_EARNFM_TOKEN` *(Settings → API Key sur app.earn.fm)* |

## Pièges connus (retour d'expérience)

- **EarnFM** : un jeton API par passerelle (`GW{n}_EARNFM_TOKEN`), token unique par device — image officielle, aucun volume.
