...
Profils compose **combinés** `gw{n}-{type}` (ex. `gw1-wipter`) + profil passerelle `gw{n}`. Types : `wipter`, `honeygain`, `packetstream`, `pawns`, `repocket` (images officielles / reconnues), ou `none`. Variable `COMPOSE_PROFILES="all|none|liste"`.
