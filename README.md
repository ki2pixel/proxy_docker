...
2. `git fetch && reset --hard origin/main` sur la VM, puis **génération des assets versionnés** (`node scripts/build-assets.mjs`, fallback sur les fichiers `dist/` commités si `node` est absent), puis `docker compose up -d --build --force-recreate --remove-orphans` avec les profils construits d'après `ENABLED_GATEWAYS` + `COMPOSE_PROFILES` du `.env` de la VM. `--force-recreate` est **obligatoire** : il ré-attache les providers (`network_mode: service:`) aux passerelles recréées — sans lui, ils resteraient dans le namespace réseau orphelin de l'ancienne passerelle (DNS local mort, devices marqués inactifs par les plateformes).
